...
function App() {
  ...
  return (
    <div className="relative min-h-screen bg-slate-900 md:bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] md:from-slate-950 md:via-slate-900 md:to-indigo-950 flex items-center justify-center p-0 md:p-6 overflow-hidden select-none">
      ...
      <div className="w-full min-h-screen md:w-[385px] md:h-[812px] md:min-h-0 bg-white rounded-none md:rounded-[52px] md:border-[12px] md:border-slate-950 shadow-none md:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8)] relative overflow-hidden flex flex-col">
        
        {/* Physical Camera Notch (iPhone Style) */}
        <div className="hidden md:flex absolute top-0 left-1/2 -translate-x-1/2 w-40 h-7 bg-slate-950 rounded-b-3xl z-50 items-center justify-center gap-1.5">
        ...
        <div className="absolute top-0 left-0 right-0 h-12 px-6 pt-3 flex items-center justify-between z-40 bg-white text-slate-800 text-xs font-semibold select-none">
          <span>9:41</span>
          <div className="flex items-center gap-1.5">
            ...
            <div className="w-5.5 h-3 border border-slate-800 rounded-sm p-0.5 flex items-center">
              ...
              <div className="w-[1.5px] h-1.5 bg-slate-800 rounded-r-xs ml-0.5"></div>
            </div>
          </div>
        </div>

        {/* Dynamic Content Body */}
        <div className="flex-1 bg-white text-slate-800 px-6 pt-14 pb-8 overflow-y-auto scrollbar-none flex flex-col relative">
          {step === 1 && (
            <UserTypeSelection
              userType={userType}
